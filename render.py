...
def render_parse(surface:cairo.Surface, parse:parse_test.TestParser, vert_per_second:float) -> None:
    ...
    page_width = queue_orig[0] + hor_per_track * (num_queues + 0.5)
    page_height = seat_orig[1] + (parse.max_t - parse.min_t) * vert_per_second
    surface.set_size(page_width, page_height)
    print(f'num_seats={num_seats}, num_queues={num_queues}, page_width={page_width}, page_height={page_height}')
    ...
    for (qid,qpos) in parse.queue_positions.items():
        ...
        hleft = queue_orig[0] + qpos*hor_per_track
        htop = 0
        hwidth = hor_per_track
    ...
    for (reqid, req) in parse.requests.items():
        ...
        for (idx,run) in enumerate(req.seat_runs):
            ...
            top=seat_orig[1] + vert_per_second*(req.real_dispatch_t-parse.min_t)
        ...
        qleft = queue_orig[0] + qpos*hor_per_track
        qtop = queue_orig[1] + vert_per_second*(req.virt_dispatch_t-parse.min_t)
        ...
        context.rectangle(qleft, qtop, qwidth, qtop)
...
if __name__ == '__main__':
    arg_parser = argparse.ArgumentParser(description='render queueset test log')
